...
public class VBS2MissionParserView extends FrameView {
        ...
        while (it.hasNext()) {
            ...
            if (nextClass.attributes.containsKey("objectType")) {
                ...
                if (nextClass.attributes.get("objectType").equals("waypoint")) {
                    ...
                    if(coords[0] > this.maxX) {
                        System.out.println("new max " + this.maxX);
                        this.maxX = coords[0];
                    }
                    else if(coords[0] < this.minX) this.minX = coords[0];

                    if(coords[1] > this.maxY) this.maxY = coords[1];
                    else if(coords[0] < this.minY) this.minY = coords[1];

                    if(coords[2] > this.maxZ) this.maxZ = coords[2];
                    else if(coords[2] < this.minZ) this.minZ = coords[2];
    ...
    private void doit() {
        ...
        double mid_x = (this.maxX-this.minX)/2.0;
        double mid_y = (this.maxY-this.minY)/2.0;
        double mid_z = (this.maxZ-this.minZ)/2.0;
        
        StringBuffer sa = new StringBuffer();
        sa.append("<scenario><id>0</id><name>unset_scenario</name>");
        sa.append("<world_center_x>" + mid_x + "</world_center_x>");
        sa.append("<world_center_y>" + mid_y + "</world_center_y>");
        sa.append("<world_center_z>" + mid_z + "</world_center_z>");
        sa.append("<scale>0</scale>");
        sa.append(sb);
        sa.append("</scenario>");
        XMLElement e = new XMLElement(sa.toString());
        ...
        JOptionPane.showMessageDialog(this.getFrame(),"Conversion Complete","Conversion complete",JOptionPane.INFORMATION_MESSAGE );
    }
