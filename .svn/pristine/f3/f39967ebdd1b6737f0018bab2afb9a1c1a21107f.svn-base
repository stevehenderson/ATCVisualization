...
class Route {
	...
	*/
	public void insertWaypoint(Waypoint wp, int position) {
		println("DEBUG: trying to add a waypoint");
		...
		for(int i=0; i < waypoints.size(); i++) {
			if(i==position) {
				//Add the new guy
				newList.add(wp);
				println("DEBUG: added a waypoint 1");
		...
		if(added==false)  {
			...
			newList.add(wp);
			println("DEBUG: added a waypoint 2");
		}
	...
	*/
	private String parseXMLString(XMLElement f, String tag) {
	  ...
	  if(e !=null) {
	  	...
	  	println("ROUTE:Didn't find tag " + tag + "..driving on..");
	  	return "no_xml_parse";
	  }
	}
	...
	*/
	public int loadFromXMLElement(XMLElement xml) {
	  ...
	  if(wElements.length > 0) {
	  	...
	  	println("ROUTE:WARNING-Failed to find any <waypoint> tags...driving on!");	  	
	  } 
